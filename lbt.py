...
def add_pepsi_params(df: pd.DataFrame, **kwargs) -> pd.DataFrame:
    ...
    df["pepsi_cd_red"] = 6
    df["pepsi_snr025_hrs"] = [
        pepsi_exptime(vmag, 25, teff=teff, fiber_setup=fiber)[cd - 1]
        for vmag, teff, fiber, cd in df[
            ["Vmag", "Teff", "pepsi_fiber", "pepsi_cd_red"]
        ].values
    ]
    df["pepsi_snr100_hrs"] = [
        pepsi_exptime(vmag, 100, teff=teff, fiber_setup=fiber)[cd - 1]
        for vmag, teff, fiber, cd in df[
            ["Vmag", "Teff", "pepsi_fiber", "pepsi_cd_red"]
        ].values
    ]
