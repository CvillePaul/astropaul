        ...
        if config_file.exists():
            ...
            if config.has_section("transformations"):
                for transformation_name, column_list in config.items("transformations"):
                    transformation_module_name = "astropaul.database.data_transformations"
                    transformation_module = importlib.import_module(transformation_module_name)
                    transformation_class = getattr(transformation_module, transformation_name, None)
                    if not (
