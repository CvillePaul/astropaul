...
def add_targets(tl: TargetList = None, **kwargs) -> TargetList:
    ...
    answer.list_criteria.add(f"All targets loaded ({len(answer.target_list)} targets)")
    return answer
...
def concat_dataframe(tl: TargetList, other_df: pd.DataFrame, **kwargs) -> TargetList:
    ...
    answer.list_criteria.add(f"Other dataframe concatenated ({len(answer.target_list)} targets)")
    return answer
...
def filter_targets(
    ...
    **kwargs,
    ...
    if inverse:
        ...
        answer.target_list = answer.target_list[criteria(answer.target_list)]
    answer.list_criteria.add(code + f" ({len(answer.target_list)} targets)")
    return answer
